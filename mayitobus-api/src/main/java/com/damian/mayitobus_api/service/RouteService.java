...
public class RouteService {
    ...
    public RouteResponse createRoute(CreateRouteRequest request) {
        if (routeRepository.existsByOriginIgnoreCaseAndDestinationIgnoreCase(request.getOrigin(), request.getDestination())) {
            throw new com.damian.mayitobus_api.exception.BusinessException("Ya existe una ruta con ese origen y destino");
        }
    ...
    public RouteResponse updateActive(Long routeId, Boolean active) {
        Route route = routeRepository.findById(routeId)
                .orElseThrow(() -> new com.damian.mayitobus_api.exception.BusinessException("No se encontro esa ruta. Actualiza la lista e intenta de nuevo"));
