...
public class BusService {
    ...
    public BusResponse createBus(CreateBusRequest request) {
        if (busRepository.existsByBusNumber(request.getBusNumber())) {
            throw new com.damian.mayitobus_api.exception.BusinessException("Ya existe un autobus con ese numero. Usa otro numero de unidad");
        }

        if (busRepository.existsByLicensePlate(request.getLicensePlate())) {
            throw new com.damian.mayitobus_api.exception.BusinessException("Ya existe un autobus con esas placas");
        }
    ...
    public BusResponse updateStatus(Long busId, String status) {
        Bus bus = busRepository.findById(busId)
                .orElseThrow(() -> new com.damian.mayitobus_api.exception.BusinessException("No se encontro ese autobus. Actualiza la lista e intenta de nuevo"));
